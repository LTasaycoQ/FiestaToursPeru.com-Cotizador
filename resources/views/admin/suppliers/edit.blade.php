...
<style>
/* ============================================================
   PROVEEDORS EDIT - ESTILOS REFINADOS
   ============================================================ */
            ...
            <div class="card-modern">
                <div class="card-header-custom">
                    <div>
                        ...
                        <div class="card-sub-custom">ID #{{ $supplier->id_supplier }}</div>
                    </div>
                </div>
                ...
                <div class="field-group">
                    ...
                    <textarea name="description" rows="3"
                              placeholder="Describe al proveedor...">{{ old('description', $supplier->description) }}</textarea>
                </div>
            </div>
            ...
            <div class="card-modern">
                ...
                <div class="form-grid-2">
                    <div class="field-group">
                        ...
                        {{-- Campo oculto para el ID del país --}}
                        <input type="hidden" name="id_country" id="edit-country-id" value="{{ old('id_country', $supplier->city->country_id ?? '') }}">
                    </div>
                    ...
                    <div class="field-group">
                        ...
                        {{-- Campo oculto para el ID de la ciudad --}}
                        <input type="hidden" name="id_cities" id="edit-ciudad-id" value="{{ old('id_cities', $supplier->id_cities) }}">
                    </div>
                </div>
            ...
            <div class="card-modern">
                ...
                <div class="field-group">
                    ...
                    <div class="chains-grid" id="chains-container">
                        @php
                            $supplierChainIds = $supplier->chains->pluck('id_chain')->toArray();
                        @endphp
                        @foreach($chains as $chain)
                            <label class="chain-checkbox {{ in_array($chain->id_chain, old('chains', $supplierChainIds)) ? 'checked' : '' }}">
                                <input type="checkbox"
                                       name="chains[]"
                                       value="{{ $chain->id_chain }}"
                                       {{ in_array($chain->id_chain, old('chains', $supplierChainIds)) ? 'checked' : '' }}
...
function cargarPaisesEdit() {
    const countryId = @json(old('id_country', $supplier->city->country_id ?? ''));
    const cityId = @json(old('id_cities', $supplier->id_cities));
    ...
    fetch(window.geoPaisesUrl)
        ...
        .then(paises => {
            ...
            if (countryId) {
                const match = opciones.find(o => String(o.value) === String(countryId));
                if (match) {
                    ...
                    // Cargar ciudades y preseleccionar la ciudad
                    cargarCiudadesEdit(countryId, cityId);
                }
            }
        })
...
function cargarCiudadesEdit(countryId, selectedCityId) {
    ...
    .then(data => {
        ...
        // Usar la ciudad seleccionada pasada como parámetro o la del proveedor
        const cityId = selectedCityId || @json(old('id_cities', $supplier->id_cities));
        if (cityId) {
            const match = opciones.find(o => String(o.value) === String(cityId));
            if (match) {
